...
Nesse tutorial usaremos conteudo baseado no Capítulo 2 do livro [Fletcher & Fortin. 2019. Spatial Ecology and Conservation Modeling: Applications with R](https://link.springer.com/book/10.1007/978-3-030-01989-1)  e ["Tutorial Escala"](https://www.needs.ufscar.br/tutorial_escala.pdf) do Dr. Alexandre Martensen.
 ...
 * Mas, sim, o objetivo é  de apresentar um tutorial mostrado os capacidades e opções para desenvolver e integrar pesquisas na ecologia da paisagem no ambiente estatística de [R](https://cran.r-project.org/)

Vamos:
1) carregar dados (raster e vectorial); 2) olhar mapas (SIG); 3) rodar alguns tarefas de geoprocessamento; 4) calcular a proporção de floresta em escalas diferentes.
...
as nossas análises. Pprecisamos os seguintes pacotes, que deve esta instalado antes: 
...
### Pergunta 1
`r kableExtra::text_spec("Com base nos resultados obtidos até agora, qual o tamanho do pixel em metros quadrados? Qual o tamnaho total da raster em hectares e quilometros quadrados?", color = "blue")`
------------------
...
```{r, dpi=300, fig.width =5, fig.height = 4, out.height=400, out.width=500, fig.cap="Paisagem simples com valores e classes de cobertura da terra."}
...
### Pergunta 2 
Confira os resultados e explique o que aconteceu.Como e porque moudou os valores em cada caso (média e modal)?
...
Precisamos carregar os dados para rios e pontos. Baixar arquivo (vector) com os dados (formato "GPKG", tamanho `r round(fs,1)` MB). 
...
O código abaixo vai carregar os dados que precisamos e criar os objetos "rsm" e "rsl.
...
O código abaixo vai carregar os dados e criar o objeto "mapbiomas_2020". Para isso trabalhamos com o pacote [terra](https://CRAN.R-project.org/package=terra). O pacote tem varios funçoes para a análise e modelagem de dados geográficos. Nós podemos ler os dados de cobertura da terra com a função <code>rast</code>.
...
### Pergunta 3
Qual é a extensão em número de pixels desse recorte?
...
Para uma comparação multiescala, vamos repetir o mesmo processo, mas com distancias de 250, 500, 1000, 2000 e 4000 metros, doubrando a escala (extensão) em cada passo. 
...
Por exemplo, nesse caso, a proporção de floresta aumenta à medida que a extensão aumenta de 500 para 4000 metros. 

### Pergunta 4
A modelagem multiescala quantifica as condições do ambiente em múltiplas escalas alterando o grão ou a extensão da análise e, em seguida, avaliando qual das escalas consideradas  explica melhor um padrão ou processo. Escolha 1 espécie aquático e 1 espécie terrestre que ocorram na região. Com base nas diferenças entre extensões e as características das espécies (por exemplo área de vida), escolher as extensões mais adequadas para um estudo multiescala de cada espécie.
