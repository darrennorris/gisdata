...
Agora vamos olhar um exemplo do mundo real. Uma pequena amostra do Rio Araguari, perto de Porto Grande. O ponto central da raster é de longitude: -51.405912 latitude: 0.726236. O arquivo tem a classificação 
da terra feito pela [MapBiomas](https://mapbiomas.org/), que produz mapeamento anual da cobertura e uso da terra desde 1985.

Baixar arquivo com os dados (formato ".tif"). 

Link: [https://github.com/darrennorris/gisdata/blob/master/inst/raster/amostra_mapbiomas_2020.tif](https://github.com/darrennorris/gisdata/blob/master/inst/raster/amostra_mapbiomas_2020.tif) .
Lembrando-se de salvar o arquivo ("amostra_mapbiomas_2020.tif")  em um local conhecido no seu computador. Lembre-se, para facilitar, os arquivos deve ficar no mesmo diretório do seu código (verifique com <code>getwd()</code>).
...
cls <- data.frame(id=classe_valor, 
...
ggplot() +
  layer_spatial(ramostra) +
  scale_fill_manual("classe", 
                      values = classe_cores) + 
     scale_x_continuous(breaks = c(-51.407,-51.406, -51.405)) +
...
Agora iremos degradar a resolução desses dados, ou seja, iremos alterar o tamanho dos pixels. Por exemplo,
iremos juntar 4 pixels em um único pixel. Como você acha que podemos fazer isso? Quais valores esse pixel
que vai substituir os 4 originais deve ter?
