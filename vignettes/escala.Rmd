...
O arquivo tem uma classificação da terra no ano 2020 feito pela [MapBiomas](https://mapbiomas.org/). Tmap (https://r-tmap.github.io/tmap-book/index.html). Visualizar para verificar.
...
tm_shape(floresta_2020_modal) +
  tm_raster(title = "Classe", style = "cat", 
            palette = c("#E974ED", "#129912")) + 
  tm_shape(rsl) + 
  tm_lines(col="blue") + 
  tm_shape(rsm) + 
  tm_dots(size = 0.4, col = "yellow") + 
tm_layout("MapBiomas 2020",
...
Para avaliar, vamos repetir o mesmo processo, mas com distancias de 250, 500, 1000, 2000 e 4000 metros (doubrando a extenção em cada passo). 

```{r, echo=FALSE, fig.width=6, fig.height=5, dpi=300, out.height=250, out.width=300}
rsm_31976 <- st_transform(rsm, 31976) 
#250 m
...
fig_1000m <- tm_shape(buffer.forest1.1km) +
 ...
 # tm_compass(position=c("left", "top")) +
tm_scale_bar(position=c("left", "bottom")) +
tm_layout(title = "raio = 1000m", 
legend.position = c("right","top"),
          legend.bg.color = "white")

tmap_arrange(fig_500m, fig_1000m)
```
